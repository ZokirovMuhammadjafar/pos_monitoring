...
public class UnAuthorizedException extends RuntimeException {
    ...
    public UnAuthorizedException(ErrorCode errorCode, Throwable throwable) {
        super(errorCode.name() + ": [NO PARAMS]", throwable);
        this.errorCode = errorCode;
        this.params = new Serializable[0];
    }

    public UnAuthorizedException(ErrorCode errorCode, List<Serializable> params) {
        super(errorCode.name() + ":" + Arrays.toString(params.toArray(new Serializable[0])));
        this.errorCode = errorCode;
        this.params = params.toArray(new Serializable[0]);
    }

    public UnAuthorizedException(ErrorCode errorCode, Serializable param) {
