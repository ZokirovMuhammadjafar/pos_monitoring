...
public interface ConstantQueries {
    ...
    String GET_MACHINES_BY_100 = """
            select t.*,
                                                                                                                   (case when checker(inst_id := t.inst_id, sr_number := t.sr_number) is false then checker(inst_id := t.inst_id, sr_number := upper( t.sr_number)) else true end )                            as is_contract,
